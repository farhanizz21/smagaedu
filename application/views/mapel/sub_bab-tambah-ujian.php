...
<?php $this->load->view('partials/header_tailwind', ['title' => 'Buat Ujian Sub Bab']); ?>
...
<div class="max-w-3xl mx-auto px-6 py-8">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
            <div class="flex items-center gap-3 mb-1">
                ...
                <h1 class="text-2xl md:text-3xl font-extrabold text-gray-900 tracking-tight">Buat Ujian Sub Bab</h1>
            </div>
            <p class="text-gray-500 text-sm ml-8">
                Materi: <span class="text-blue-600 font-semibold"><?= $materi->judul ?></span> →
                Sub Bab: <span class="text-blue-600 font-semibold"><?= $bab->judul ?></span>
            </p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 p-6 md:p-8 table-shadow">
        <?= form_open('sub_bab/tambah_ujian/' . $bab->uuid); ?>
        <div class="space-y-6">
            <div class="bg-blue-50 rounded-xl p-4 border border-blue-100 flex items-center gap-2">
                <i data-lucide="info" class="w-4 h-4 text-blue-600 flex-shrink-0"></i>
                <p class="text-xs text-blue-600 font-medium">
                    Ujian akan otomatis terhubung dengan sub bab "<?= $bab->judul ?>"
                </p>
            </div>
            ...
            <div>
                ...
                <select name="jenis_penilaian"
                    ...
                    <option value="penilaian_tengah_semester"
                        <?= set_value('jenis_penilaian') == 'penilaian_tengah_semester' ? 'selected' : '' ?>>Penilaian
                        Tengah Semester</option>
                    <option value="penilaian_akhir_semester"
                        <?= set_value('jenis_penilaian') == 'penilaian_akhir_semester' ? 'selected' : '' ?>>Penilaian
