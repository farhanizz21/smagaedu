...
<?php $this->load->view('partials/header_tailwind', ['title' => 'Tambah Guru']); ?>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
            <div class="flex items-center gap-3 mb-1">
                ...
                <h1 class="text-2xl md:text-3xl font-extrabold text-gray-900 tracking-tight">Tambah Data Guru</h1>
            </div>
            <p class="text-gray-500 text-sm ml-8">Lengkapi formulir di bawah untuk menambahkan guru baru</p>
        </div>
    </div>

    <!-- Form Card -->
    <div class="bg-white rounded-2xl border border-gray-200 p-6 md:p-8 table-shadow">
        <form method="post" action="<?= base_url('guru/tambah');?>" id="guruForm">
            ...
            <div class="mt-8">
                ...
                <div id="mapelKelasContainer" class="space-y-3">
                    ...
                    <p id="emptyMapelMessage"
                        class="text-sm text-gray-400 italic py-4 text-center border-2 border-dashed border-gray-200 rounded-xl">
                        Belum ada mata pelajaran dipilih. Klik "Tambah Pelajaran" untuk menambahkan.
                    </p>
                </div>
...
$(document).ready(function() {
    ...
    $('#confirmMapelBtn').on('click', function() {
        ...
        $('.mapel-option-checkbox:checked').each(function() {
            var uuid = $(this).val();
            var nama = $(this).closest('.mapel-option').data('nama');
    ...
    $(document).on('click', '.tambah-kelas-btn', function() {
        ...
        // Find current kelas selections for this mapel
    ...
    $('#guruForm').on('submit', function(e) {
        ...
        // Add hidden inputs for mapel and kelas
    ...
    function restoreFromOldInput(oldMapels, oldKelasMap) {
        oldMapels.forEach(function(uuid) {
            var nama = '';
            mapelData.forEach(function(m) {
                if (m.uuid === uuid) nama = m.nama;
            });
            var kelasList = [];
            if (oldKelasMap[uuid]) {
                oldKelasMap[uuid].forEach(function(kelasUuid) {
                    var kelasNama = '';
