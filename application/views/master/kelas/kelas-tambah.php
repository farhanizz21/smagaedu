    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
            <div class="flex items-center gap-3 mb-1">
                <a href="<?= base_url('kelas')?>" class="text-gray-400 hover:text-gray-600 transition-colors">
                    <i data-lucide="arrow-left" class="w-5 h-5"></i>
                </a>
                <h1 class="text-2xl md:text-3xl font-extrabold text-gray-900 tracking-tight">Tambah Data Kelas</h1>
            </div>
            <p class="text-gray-500 text-sm ml-8">Lengkapi formulir di bawah untuk menambahkan kelas baru</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 p-6 md:p-8 table-shadow">
        <form method="post" action="<?= base_url('kelas/tambah');?>">
            <div class="grid md:grid-cols-1 gap-6">
                <!-- Nama Kelas -->
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1.5">Nama Kelas <span
                    ...
                    <input type="text" name="namaKelas" id="namaKelas"
                        ...
                        placeholder="Masukkan Nama Kelas (contoh: X-A, XI-B, XII-C)"
                        value="<?= set_value('namaKelas'); ?>">
                    <div class="text-red-500 text-xs mt-1 <?= !empty(form_error('namaKelas')) ? '' : 'hidden' ?>">
                        <?= form_error('namaKelas') ?>
                    </div>
                </div>
            </div>
